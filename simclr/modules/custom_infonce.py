...
class Custom_InfoNCE(nn.Module):
    ...
    def forward(self, anchor_rec, positive_rec):
        ...
        if not self.bound:
            ...
            list_index=[idx for idx in list(range(sim11.shape[0]*sim11.shape[1]*sim11.shape[2])) if (idx-((idx//(sim11.shape[0]*sim11.shape[-1]))*(sim11.shape[0]*sim11.shape[-1])))%sim11.shape[0] != ((idx-((idx//(sim11.shape[0]*sim11.shape[-1]))*(sim11.shape[0]*sim11.shape[-1])))//sim11.shape[0]) else -1].remove(-1)
